...
## Consequences
- Depth Dive's response needs a field or mechanism to carry this "search was attempted and failed" signal through to the rendered output — likely part of Depth Dive's eventual structured response schema (parallel to Retrieval QA's `grounded` field in ADR-0009), left as an implementation detail for when that schema is designed.
...
- This is consistent with Retrieval QA's `grounded: false` pattern (ADR-0009): both modules prefer telling the user "this is a lower-confidence result" over silently presenting a degraded response as if it were the intended one.