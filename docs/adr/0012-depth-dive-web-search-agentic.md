...
- The safer quotation pattern needs to be enforced in the system prompt and ideally checked (similar in spirit to Retrieval QA's `grounded` field) — a structured Depth Dive response type that tracks quote length and source attribution would let this be verified deterministically rather than trusted to prompt compliance alone. Left open as an implementation detail for when Depth Dive's response schema is designed.
