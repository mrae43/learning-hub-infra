...
## Depth Dive
The synthesis harness that consumes the shared retrieval layer (ADR-0019) and produces a self-contained, interactive explanation of a single Captured Passage. The MVP output is one **interactive_animation** scene graph — a declarative, stateless payload pairing explanatory text with a visual (dual coding) — generated deterministically from templates rather than by the LLM (ADR-0022) and rendered to self-contained HTML the client opens without further server calls. The MVP client is a Claude Code plugin, not a web UI (ADR-0023). Treatments such as `worked_example`, `prediction_reveal`, and `segmented_carousel` are layered onto the animation. Unlike Retrieval QA, Depth Dive is permitted to extend beyond the ingested corpus via agentic web search (ADR-0012, ADR-0013). Stateless; no quizzing, no scheduling, no per-learner state in this repo. Ships after Retrieval QA is proven, since it depends on shared retrieval quality.
...
## Traces Path
The trace-observation arm of the observability stack: the same OTel Collector fans the api's spans out to self-hosted Langfuse via an `otlphttp` exporter on the v4 OTLP endpoint (`/api/public/otel`, Basic-Auth with the project's `pk-lf-…`/`sk-lf-…` keys + `x-langfuse-ingestion-version: 4`). Langfuse runs behind the `observability` compose profile with its own dedicated Postgres container/database/user — never the app's pgvector database — plus ClickHouse, Redis, and MinIO backing stores. Shares the api's single OTLP/HTTP export with the Metrics Path (one instrumentation path, two backends); like it, the Traces Path is inert unless the profile is started. Distinct from the Metrics Path (Prometheus/Grafana RED metrics) — both consume the same spans, differing only in the destination.