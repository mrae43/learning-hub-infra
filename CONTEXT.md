...
## Captured Passage
The specific content a user selects or pastes while reading to anchor a Depth Dive request. One of five passage types — **text, image, diagram, table, or code snippet** — each defined by a boundary model discriminated on `passage_type` (ADR-0014's `chunk_id` remains the canonical anchor for the text and code forms). The content is carried with the request rather than re-fetched from the store. Corpus anchoring is **optional**: a captured passage may be unanchored (e.g. a code snippet pasted from a library's official docs), and the harness similarity-checks unanchored passages against the ingested corpus before producing output. Distinct from "injected context" — a captured passage is user-selected and explicit, not retrieved; distinct from a "chunk" — a chunk is system-produced and grounds Retrieval QA, a captured passage grounds Depth Dive.
